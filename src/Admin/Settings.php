...
final class Settings implements HasHooks
{
    ...
    public function addMenuPage(): void
    {
        add_submenu_page(
            'woocommerce',
            __('Gift Registries', 'plogins-registry'),
            __('Gift Registries', 'plogins-registry'),
            'manage_woocommerce',
            self::PAGE,
            [$this, 'renderPage'],
        ...
        <div class="wrap registry-admin">
            ...
            <form method="post" action="options.php">
                ...
                <div class="registry-card">
                    <h2><?php esc_html_e('Storefront visibility', 'plogins-registry'); ?></h2>
                    <p class="registry-card__lead"><?php esc_html_e('Controls whether the registry feature appears to customers and guests. Both options are on by default, so registries work the moment the plugin is active, no setup required.', 'plogins-registry'); ?></p>
                    <table class="form-table" role="presentation">
                        <tbody>
                            <tr>
                                <th scope="row"><?php esc_html_e('Enable gift registries', 'plogins-registry'); ?></th>
                                <td>
                            ...
                            <tr>
                                <th scope="row"><?php esc_html_e('Allow direct purchase', 'plogins-registry'); ?></th>
                                <td>
                                    <label for="registry_allow_purchase">
                                        ...
                                        <?php esc_html_e('Let guests buy registry items straight from the shared page.', 'plogins-registry'); ?>
                                    </label>
                                    <p class="description"><?php esc_html_e('On (recommended): a guest clicks "Buy this gift" on the registry and the item drops into their cart in one step. Gifts that need options picked first, such as a variable product with a size or colour, always show "Choose options" and open the product page instead, the registry is remembered so the gift still counts. Off: every item sends guests to the product page. Has no effect unless gift registries are enabled above.', 'plogins-registry'); ?></p>
                                    <p class="registry-example">
                                        <span class="registry-example__label"><?php esc_html_e('Guests reach a registry at a link like:', 'plogins-registry'); ?></span>
                                        <code class="registry-example__code"><?php echo esc_html(trailingslashit(home_url()) . _x('registry/emma-and-sam-wedding/', 'example registry share URL slug', 'plogins-registry')); ?></code>
                                    </p>
                                </td>
                            </tr>
                        </tbody>
